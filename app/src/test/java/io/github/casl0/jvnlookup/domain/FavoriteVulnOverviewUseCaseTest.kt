...
class FavoriteVulnOverviewUseCaseTest {
    ...
    fun invoke_FavoriteTrue_LocalDataUpdated() = runTest {
        ...
        useCase(id = targetId, favorite = true)

        val resultList = useCase.favorites.first()
        assertTrue(resultList.first { it.id == targetId }.isFavorite)
    }
    ...
    fun invoke_LocalDataUpdated() = runTest {
        ...
        val job = launch(UnconfinedTestDispatcher()) {
            useCase.favorites.collect {
                resultLocalData = it
            }
        }

        useCase(vulnOverview = newData, favorite = true)
        assertTrue(resultLocalData.first { it.id == newData.id }.isFavorite)

        useCase(vulnOverview = newData, favorite = false)
        assertFalse(resultLocalData.first { it.id == newData.id }.isFavorite)
